...
import static sparklemod.util.GeneralUtils.removePrefix;
import static sparklemod.util.TextureLoader.getCardTextureString;


public abstract class BaseCard extends CustomCard {
    final private static Map<String, DynamicVariable> customVars = new HashMap<>();

    protected static String makeID(String name) { return SparkleMod.makeID(name); }
    ...
    public void upgrade()
    {
        if (!upgraded)
        {
            ...
            if (this.upgradesDescription)
            {
                ...
                {
                    SparkleMod.logger.error("Card " + cardID + " upgrades description and has null upgrade description.");
                }
                else
                {
    ...
    public void updateCost (int amount) {
        int tempAmount;
        if(cost + amount > AbstractDungeon.player.energy.energy) {
            tempAmount = AbstractDungeon.player.energy.energy - cost;
        }
        else {
            tempAmount = amount;
        }
